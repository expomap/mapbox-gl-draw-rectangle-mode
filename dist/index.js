...
var DrawRectangle = {
  ...
  onMouseMove: function onMouseMove(state, e) {
    ...
    if (state.startPoint) {
      var startPointInPx = state.rectangle.ctx.map.project([state.startPoint[0], state.startPoint[1]]);
      var currentPointInPx = state.rectangle.ctx.map.project([e.lngLat.lng, e.lngLat.lat]);
      var pontoDireita = state.rectangle.ctx.map.unproject([currentPointInPx.x, startPointInPx.y]);
      var pontoBaixo = state.rectangle.ctx.map.unproject([startPointInPx.x, currentPointInPx.y]);
      state.rectangle.updateCoordinate("0.0", state.startPoint[0], state.startPoint[1]); //minX, minY - the starting point

      state.rectangle.updateCoordinate("0.1", pontoDireita.lng, pontoDireita.lat); // maxX, minY

      state.rectangle.updateCoordinate("0.2", e.lngLat.lng, e.lngLat.lat); // maxX, maxY

      state.rectangle.updateCoordinate("0.3", pontoBaixo.lng, pontoBaixo.lat); // minX,maxY
